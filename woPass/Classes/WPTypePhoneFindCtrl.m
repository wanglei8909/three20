...
@implementation WPTypePhoneFindCtrl
{
...
- (void)viewDidLoad{
    ...
    mScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 74, SCREEN_WIDTH, SCREEN_HEIGHT)];
    ...
    NSArray *placeholderText = @[@"请输入联通手机号",@"请输入随机密码",@"请输入新密码"];
    for (int i = 0; i<3; i++) {
        float top = 5;
        ...
        if (code == 0  && responseObject) {
            ...
            WPAlertView *av = [[WPAlertView alloc] initWithTitle:@"设置成功"
                                                    ...
                                                    buttonTitles:@[@"确定"] andCancleClick:^{
                                                        for (UIViewController *ctrl in weakSelf.navigationController.viewControllers) {
