...
FBKVONotificationBlock blockBrige(TZKVONotificationBlock tzBlock) {
    return ^ (id observer, id object, NSDictionary *change) {
        
        tzBlock(observer, object, change[@"old"], change[@"new"]);
    };
}

- (void)observe:(id)object keyPath:(NSString *)keyPath block:(TZKVONotificationBlock)block {
    
    [self observe:object keyPath:keyPath options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld block:blockBrige(block)];
}
...
- (void)observe:(id)object keyPaths:(NSArray *)keyPaths block:(TZKVONotificationBlock)block {
    
    [self observe:object keyPaths:keyPaths options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld block:blockBrige(block)];
}
