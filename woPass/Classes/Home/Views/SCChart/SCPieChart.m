...
- (UIView *) createInnerView{
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, _innerCircleRadius*2+30, _innerCircleRadius*2+30)];
    view.backgroundColor = [UIColor blackColor];
    view.layer.masksToBounds = YES;
    view.layer.cornerRadius = (_innerCircleRadius*2+30)/2;
    view.alpha = 0.2;
    view.center = CGPointMake(self.frame.size.width/2, self.frame.size.height/2);
    
    
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 100, _innerCircleRadius)];
    label.textAlignment = NSTextAlignmentCenter;
    label.textColor = [UIColor whiteColor];
    label.text = @"最近二十次";
    label.center = CGPointMake(self.width*0.5, self.width*0.5-10);
    [self addSubview:label];
    
    label = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, _innerCircleRadius, _innerCircleRadius)];
    label.textAlignment = NSTextAlignmentCenter;
    label.textColor = [UIColor whiteColor];
    label.text = @"登录";
    label.center = CGPointMake(self.width*0.5, self.width*0.5+10);
...
- (void)loadDefault{
    ...
    _lineLayer.frame = CGRectMake(0, 0, 200, 200);
...
- (UILabel *)descriptionLabelForItemAtIndex:(NSUInteger)index{
    ...
    CGFloat distance = _outerCircleRadius + 50;
    ...
    descriptionLabel.text =  [NSString stringWithFormat:@"%@",titleText];
    
    NSString *str = [NSString stringWithFormat:@"\n%.0f%%",[self ratioForItemAtIndex:index] * 100];
    
    descriptionLabel.text = [descriptionLabel.text stringByAppendingString:str];
    
    //向下偏移量
    double yDis = 0;
    double yH = 15;
    if (centerPercentage>=0 && centerPercentage<0.25) {
        yDis = yH * centerPercentage * 4 * -1;
    }else if (centerPercentage>=0.25 && centerPercentage<0.5){
        yDis = yH * (0.5 - centerPercentage) * 4;
    }else if (centerPercentage>=0.5 && centerPercentage<0.75){
        yDis = yH * (centerPercentage - 0.5) * 4;
    }else if (centerPercentage>=0.75 && centerPercentage<=1){
        yDis = yH * (centerPercentage - 1) * 4;
    }
    
    CGPoint center = CGPointMake(_outerCircleRadius + distance * sin(rad),
                                 _outerCircleRadius - distance * cos(rad) + yDis);
    ...
    descriptionLabel.textColor       = currentDataItem.color;
...
- (void)drawLine{
    for (int i = 0; i < _items.count; i++) {
        SCPieChartDataItem *currentDataItem = [self dataItemForIndex:i];
        CGFloat distance1 = _outerCircleRadius;
        CGFloat distance2 = _outerCircleRadius + 20;
        CGFloat centerPercentage = ([self startPercentageForItemAtIndex:i] + [self endPercentageForItemAtIndex:i])/ 2;
        CGFloat rad = centerPercentage * 2 * M_PI;
        CGPoint center1 = CGPointMake(_outerCircleRadius + distance1 * sin(rad),
                                     _outerCircleRadius - distance1 * cos(rad));
        CGPoint center2 = CGPointMake(_outerCircleRadius + distance2 * sin(rad),
                                      _outerCircleRadius - distance2 * cos(rad));
        
        CAShapeLayer *line = [CAShapeLayer layer];
        UIBezierPath *path = [UIBezierPath bezierPath];
        // 添加路径[1条点(100,100)到点(200,100)的线段]到path
        [path moveToPoint:CGPointMake(center1.x , center1.y)];
        [path addLineToPoint:CGPointMake(center2.x, center2.y)];
        line.strokeColor = currentDataItem.color.CGColor;
        line.lineWidth   = 1;
        line.path        = path.CGPath;
        
        [_lineLayer addSublayer:line];
        
        UIBezierPath *pathCircle = [UIBezierPath bezierPath];
        // 添加圆到path
        [pathCircle addArcWithCenter:center2 radius:3 startAngle:0.0 endAngle:M_PI*2 clockwise:YES];
        //设置颜色（颜色设置也可以放在最上面，只要在绘制前都可以）
        //[[UIColor redColor] setFill];
        
        CAShapeLayer *circle = [CAShapeLayer layer];
        circle.path = pathCircle.CGPath;
        circle.fillColor = currentDataItem.color.CGColor;
        [_lineLayer addSublayer:circle];
    }
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag{
    ...
    _innerView = [self createInnerView];
    [self addSubview:_innerView];
    [UIView animateWithDuration:0.5 animations:^{
        _innerView.transform = CGAffineTransformScale(_innerView.transform,1.2,1.2);
    }completion:^(BOOL finish){
        [UIView animateWithDuration:0.5 animations:^{
            _innerView.transform = CGAffineTransformScale(_innerView.transform, 0.83, 0.83);
        }completion:^(BOOL finish){
