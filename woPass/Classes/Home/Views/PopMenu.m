...
- (UIButton *)endButton {
    if (!_endButton) {
        ...
        [_endButton addTarget:self action:@selector(dismissMenu) forControlEvents:UIControlEventTouchUpInside];
        _endButton.frame = CGRectMake(0, 0, 21, 21);
        _endButton.center = CGPointMake(SCREEN_WIDTH*0.5, SCREEN_HEIGHT-80);
        _endButton.hidden = YES;
        [_endButton setImage:[UIImage imageNamed:@"saoraoguanbi"] forState:UIControlStateNormal];
...
- (void)setup {
    ...
    _realTimeBlur.didShowBlurViewcompleted = ^(BOOL finished){
        weakSelf.endButton.hidden = NO;
    };
