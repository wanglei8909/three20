...
- (void)timeout {
    CGFloat delta = 2;    
    NSString *string;
    if (fabs(self.currentIndicatorPercent - (self.toValue * 100 / self.fromValue)) < delta) {
        
        [self.displayLink invalidate];
        self.displayLink = nil;
        
        if (self.currentIndicatorPercent < 1) {
            
            self.currentIndicatorPercent = 1;
        }
        
        if (self.currentIndicatorPercent > 99) {
        ...
        self.currentIndicatorPercent -= delta;
    }
...
//    result = [self HSBGradualWithPercent:percent];
    if (percent > 10) {
        ...
        result = RGBCOLOR_HEX(kStartColor);
    } else {
        
        result = RGBCOLOR_HEX(kEndColor);
    }
...
- (void)drawRect:(CGRect)rect {
    ...
    point = CGPointMake(point.x - 10, point.y - SCALED(15));
    text = @"50%";
    [text drawInRect:CGRectMake(point.x, point.y, 100, 100) withAttributes:@{
                                                                             NSFontAttributeName : XFont(kFontTiny),
                                                                             NSForegroundColorAttributeName : [UIColor whiteColor]
