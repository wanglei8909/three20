...
@implementation WPComCityViewController
...
- (void)CommitClick{
    if (self.finishBlock) {
        ...
        [_haveSelectedView.cityArray enumerateObjectsUsingBlock:^(NSDictionary *cityDic, NSUInteger idx, BOOL *stop) {
            
            [cityNameArray addObject:cityDic[@"name"]];
        }];
        ...
        self.finishBlock(cityNameArray);
    }
}
- (void)mCancle{
    [self dismiss];
...
- (void)viewDidLoad{
    [super viewDidLoad];
    [self AddRightTextBtn:@"完成" target:self action:@selector(CommitClick)];
    [self AddLeftTextBtn:@"取消" target:self action:@selector(mCancle)];
    ...
    _haveSelectedView = [[WPComCityShowView alloc]init];    
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    if (tableView == _cityTable && section>0) {
        return 35;
    }
    return 0;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == _cityTable) {
        if (indexPath.section >1) {
            NSDictionary *dict = _cityArray[indexPath.section-1][@"cities"][indexPath.row];
            [self OnCheck:dict];
        }
    }else{
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (tableView == _cityTable) {
        ...
        return [self.cityArray[section-1][@"cities"] count];
    }
    else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == _cityTable) {
        if (indexPath.section == 0) {
            ...
            [cell.contentView addSubview:_haveSelectedView];
            return cell;
        }
        else if (indexPath.section == 1) {
            static NSString *cellId = @"WPCityCell";
            ...
            cell.textLabel.text = _cityArray[indexPath.section-1][@"cities"][indexPath.row][@"name"];
            return cell;
        }
    }else{
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (section==1) {
        ...
        title = _cityArray[section-1][@"key"];
    }
    label.text = title;
    return view;
}
...
- (void)OnCheck:(NSDictionary *)cDict{
    [_haveSelectedView AddCity:cDict];
}

-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    if (scrollView.contentOffset.y>44) {
        if (scrollView.top<64+10) {
            scrollView.frame  = CGRectMake(0, 64+80, SCREEN_WIDTH, SCREEN_HEIGHT - (64+80));
            scrollView.contentOffset = CGPointMake(0, 44+80);
            [self.view addSubview:_haveSelectedView];
            _haveSelectedView.frame = CGRectMake(0, 64, SCREEN_WIDTH, 80);
            _haveSelectedView.backgroundColor = [UIColor whiteColor];
        }
    }
    if (scrollView.contentOffset.y<44+80){
        if (scrollView.top>64+10) {
            scrollView.frame  = CGRectMake(0, 64, SCREEN_WIDTH, SCREEN_HEIGHT - 64);
            scrollView.contentOffset = CGPointMake(0, 44);
            _haveSelectedView.frame = CGRectMake(0, 0, SCREEN_WIDTH, 80);
            _haveSelectedView.backgroundColor = [UIColor clearColor];
        }
    }
}
